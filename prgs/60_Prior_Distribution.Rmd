...
```{r}
p1_predict <- posterior_epred(object = prior_p1, nsamples = 100)
...
```
```{r}
p2_predict <- posterior_epred(object = prior_p2, nsamples = 100)
...
```
```{r}
p3_predict <- posterior_epred(object = prior_p3, nsamples = 100)
...
```
```{r}
p4_predict <- posterior_epred(object = prior_p4, nsamples = 100)
...
ggsave("../figures/lsat_predicted.png", lsat_predicted, width = 5, height = 3, units = "in")
```
...
p_sd_sim <-
  ...
  geom_line() +
  scale_x_continuous(breaks = seq(from = -10, to = 10, by = 1)) +
  labs(title = "sd ~ dcauchy(0, 1)", y = "density")
p_sd_sim
