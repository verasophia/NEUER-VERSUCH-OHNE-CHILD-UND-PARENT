...
test <-
  ...
  arrange(row, lsat) %>% 
  mutate(prior = ifelse(row == "flat", dnorm(lsat, 5, 200), dnorm(lsat, 7, 1)),
...
ggsave("../figures/flat_peaked.png", flat_peaked, width = 7, height = 4, units = "in")
flat_peaked
```
...
```{r}
m1.1 <- brm( lsat ~ treated + welle  + tXw2 + tXw3 + tXw4 + tXw5 + female + migback + hob.mus.making + (1 + treated + tXw2 + tXw3 + tXw4 + tXw5 | school) + (1 | pid),
           sample_prior = "only", data = d,
           prior = c(prior(normal(0, 5), class = Intercept),
                     prior(normal(0, 0.75), class = b),
                     prior(cauchy(0, 1), class = sd)),
           file = "../results/prior_m11")

m1.2 <- brm( lsat ~ treated + welle  + tXw2 + tXw3 + tXw4 + tXw5 + female + migback + hob.mus.making + (1 + treated + tXw2 + tXw3 + tXw4 + tXw5 | school) + (1 | pid),
           sample_prior = "only", data = d,
           prior = c(prior(normal(0, 5), class = Intercept),
                     prior(normal(0, 1), class = b),
                     prior(cauchy(0, 1), class = sd)),
           file = "../results/prior_m12")

m1.3 <- brm( lsat ~ treated + welle  + tXw2 + tXw3 + tXw4 + tXw5 + female + migback + hob.mus.making + (1 + treated + tXw2 + tXw3 + tXw4 + tXw5 | school) + (1 | pid),
           sample_prior = "only", data = d, # If sample_prior is set to "only", samples are drawn solely from the priors ignoring the likelihood, which allows among others to generate samples from the prior predictive distribution. In this case, all parameters must have proper priors.
           prior = c(prior(normal(0, 5), class = Intercept),
                     prior(normal(0, 10), class = b),
                     prior(cauchy(0, 1), class = sd),
                     prior(exponential(0.5), class = sigma)),
           file = "../results/prior_m13")

m2.1 <- brm( lsat ~ treated + welle  + tXw2 + tXw3 + tXw4 + tXw5 + female + migback + hob.mus.making + (1 + treated + tXw2 + tXw3 + tXw4 + tXw5 | school) + (1 | pid),
           sample_prior = "only", data = d,
           prior = c(prior(normal(7, 2), class = Intercept),
                     prior(normal(0, 0.8), class = b),
                     prior(cauchy(0, 1), class = sd),
                     prior(exponential(0.5), class = sigma)),
                     file = "../results/prior_m21")

m2.2 <- brm( lsat ~ treated + welle  + tXw2 + tXw3 + tXw4 + tXw5 + female + migback + hob.mus.making + (1 + treated + tXw2 + tXw3 + tXw4 + tXw5 | school) + (1 | pid),
           sample_prior = "only", data = d,
           prior = c(prior(normal(7, 2), class = Intercept),
                     prior(normal(0, 1), class = b),
                     prior(cauchy(0, 1), class = sd),
                     prior(exponential(0.5), class = sigma)),
           file = "../results/prior_m22")

m2.3 <- brm( lsat ~ treated + welle  + tXw2 + tXw3 + tXw4 + tXw5 + female + migback + hob.mus.making + (1 + treated + tXw2 + tXw3 + tXw4 + tXw5 | school) + (1 | pid),
           sample_prior = "only", data = d,
           prior = c(prior(normal(7, 2), class = Intercept),
                     prior(normal(0, 10), class = b),
                     prior(cauchy(0, 1), class = sd),
                     prior(exponential(0.5), class = sigma)),
           file = "../results/prior_m23")

m2.4 <- brm( lsat ~ treated + welle  + tXw2 + tXw3 + tXw4 + tXw5 + female + migback + hob.mus.making + (1 + treated + tXw2 + tXw3 + tXw4 + tXw5 | school) + (1 | pid),
           sample_prior = "only", data = d,
           prior = c(prior(normal(7, 2), class = Intercept),
                     prior(normal(0, 100), class = b),
                     prior(cauchy(0, 1), class = sd),
                     prior(exponential(0.5), class = sigma)),
           file = "../results/prior_m24")
```

```{r}
readRDS("../resuls/prior_m11.rds")
readRDS("../resuls/prior_m12.rds")
readRDS("../resuls/prior_m13.rds")
readRDS("../resuls/prior_m21.rds")
readRDS("../resuls/prior_m22.rds")
readRDS("../resuls/prior_m23.rds")
readRDS("../resuls/prior_m23.rds")
```

```{r}
m1.1_predict <- posterior_epred(object = m1.1, nsamples = 100)
m1.1_long <- gather(data.frame(m1.1_predict))
m1.1_long
prior11 <- ggplot(m1.1_long, aes(x=value, group=key)) +
  geom_histogram(alpha=.2, bins = 200, fill = "#011f4b") +
  xlim(-50,50) +
  xlab("Predicted life satisfaction") + 
  ylab("Density") +
  labs(title = "prior(normal(0, 5), class = Intercept),
                prior(normal(0, 1), class = b") +
  theme(axis.text.y = element_blank())
```
```{r}
m1.2_predict <- posterior_epred(object = m1.2, nsamples = 100)
m1.2_long <- gather(data.frame(m1.2_predict))
m1.2_long
prior12 <- ggplot(m1.2_long, aes(x=value, group=key)) +
  geom_histogram(alpha=.2, bins = 200, fill = "#011f4b") +
  xlim(-50,50) +
  xlab("Predicted life satisfaction") + 
  ylab("Density") +
...
```
```{r}
m1.3_predict <- posterior_epred(object = m1.3, nsamples = 100)
m1.3_long <- gather(data.frame(m1.3_predict))
m1.3_long
prior13 <- ggplot(m1.3_long, aes(x=value, group=key)) +
  geom_histogram(alpha=.2, bins = 200, fill = "#011f4b") +
  xlim(-50,50) +
  xlab("Predicted life satisfaction") + 
  ylab("Density") +
...
```
```{r}
m2.1_predict <- posterior_epred(object = m2.1, nsamples = 100)
m2.1_long <- gather(data.frame(m2.1_predict))
m2.1_long
prior21 <- ggplot(m2.1_long, aes(x=value, group=key)) +
  geom_histogram(alpha=.2, bins = 200, fill = "#011f4b") +
  xlim(-50,50) +
  xlab("Predicted life satisfaction") + 
  ylab("Density") +
  labs(title = "prior(normal(0, 0.8), class = b") +
  theme(axis.text.y = element_blank())
prior21
```
```{r}
m2.2_predict <- posterior_epred(object = m2.2, nsamples = 100)
m2.2_long <- gather(data.frame(m2.2_predict))
m2.2_long
prior22 <- ggplot(m2.2_long, aes(x=value, group=key)) +
  geom_histogram(alpha=.2, bins = 200, fill = "#011f4b") +
  xlim(-50,50) +
  xlab("Predicted life satisfaction") + 
  ylab("Density") +
  labs(title = "prior(normal(0, 1), class = b") +
  theme(axis.text.y = element_blank())
```
```{r}
m2.3_predict <- posterior_epred(object = m2.3, nsamples = 100)
m2.3_long <- gather(data.frame(m2.3_predict))
m2.3_long
prior23 <- ggplot(m2.3_long, aes(x=value, group=key)) +
  geom_histogram(alpha=.2, bins = 200, fill = "#011f4b") +
  xlim(-50,50) +
  xlab("Predicted life satisfaction") + 
  ylab("Density") +
  labs(title = "prior(normal(0, 10), class = b") +
  theme(axis.text.y = element_blank())
```
```{r}
m2.4_predict <- posterior_epred(object = m2.4, nsamples = 100)
m2.4_long <- gather(data.frame(m2.4_predict))
m2.4_long
prior24 <- ggplot(m2.4_long, aes(x=value, group=key)) +
  geom_histogram(alpha=.2, bins = 200, fill = "#011f4b") +
  xlim(-50,50) +
  xlab("Predicted life satisfaction") + 
  ylab("Density") +
  labs(title = "prior(normal(0, 100), class = b") +
  theme(axis.text.y = element_blank())
```
```{r}
lsat_predicted <- grid.arrange(prior21, prior22, prior23, prior24, ncol = 2)
