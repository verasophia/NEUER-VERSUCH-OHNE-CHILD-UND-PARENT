...
peaked <-
  ...
  labs(x="", y="") +
  theme(text = element_text(size=14))
flat_peaked <- flat / peaked + ylab(Density)
...
prior_predict <- ggplot(all, aes(x=value, y=..density..)) +
  ...
  prior = c("1" = "N(0,10)", "2" = "N(0,5)", "3" = "N(0,1)"))) +
...
p_sigma_sim <-
  ...
  geom_line(color='#6497b1') +
  scale_x_continuous(breaks = seq(from = -10, to = 10, by = 2)) +
  labs(title = "sigma ~ dexp(0.5)", x="", y="") +
  theme(text = element_text(size=14))
p_sigma_sim

plot_priors <- grid.arrange(p_intercept_sim, p_b_sim, p_sd_sim, p_sigma_sim, left="Density", bottom="x")
