...
std <- d %>% 
  group_by(pid) %>%
  summarise( mean.sat.pid = mean(lsat, na.rm = TRUE),
             sd.sat.pid=sd(lsat, na.rm=TRUE) )
std <- std[!(std$sd.sat.pid >= 2.5),]
std <- na.omit(std)

d1 <- merge(std, d, by = 'pid')

sample10 <- sample(1:394, 10, replace=FALSE, set.seed(5))
sample10 <- sample(1:394, 10, replace=FALSE, set.seed(20))
set.seed(Sys.time())
sample8 <- sample(1:394, 8, replace=FALSE)
print(sample8)

pid10 <- d %>% filter(pid==10 | pid==29)
pid10 <- d %>% filter(pid==sample10[1] |
                        pid==sample10[2] |
                        pid==sample10[3] |
                        pid==sample10[4] |
                        pid==sample10[5] |
...
random_lsat <- ggplot(
  data = pid8,
  aes(y= lsat, x= welle, group = pid)) +
  geom_line(aes(color=pid), size = 1.6) +
  labs(colour = "PID", x = "Wave", y = "Life satisfaction") +
  theme(text = element_text(size=16))
random_lsat

ggsave("../figures/random_lsat.png", random_lsat, 
       width = 7, height = 4, units = "in")
  
  geom_smooth(aes(color=pid), method = "lm", se = F, size = .4, linetype = "twodash")
...
m2 <- brm( lsat ~ treated + welle  + tXw2 + tXw3 + tXw4 + tXw5 + 
             female + migback + hob.mus.making +
             (1 + treated + tXw2 + tXw3 + tXw4 + tXw5 | school) + (1 | pid),
           prior = prior, cores = 4, chains = 4, 
           data = dd, seed=123,
...
gr_lsat_teff <- ggplot(data = su_teff, 
       aes(x = wave, 
           y = mean)) +
  geom_pointrange(aes(ymin = Q10, ymax = Q90), colour = "#6497b1") +
  scale_x_discrete("Wave", limits=c("2", "3", "4", "5")) +
  geom_hline(yintercept=0, colour = "#6497b1") +
...
gr_lsat_teff_across_schools <-
  ...
  geom_pointrange(aes(color = school, group = school),
                  position=position_dodge(width=0.40)) +
  geom_hline(yintercept=0) + 
  theme(legend.position="bottom") +
  theme(text = element_text(size = 16),
...
gr_sat_school_teff <- ggplot(data = su_teff, 
  ...
  ylab("Average treatment effect") +
  theme(text = element_text(size = 16))
ggsave("../figures/sat_school_teff.png", gr_lsat_teff, width = 7, height = 4, units = "in")
gr_sat_school_teff
```
