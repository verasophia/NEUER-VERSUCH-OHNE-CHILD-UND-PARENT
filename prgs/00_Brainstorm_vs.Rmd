...
d <- d %>% select( school, pid, welle, lsat, sat_friends, sat_class, treated, female, classsit.prob, classsit.excl, note.deu, note.mat, note.mus, hob.mus.making, hob.mus.lis, dauer.mus.lis, dauer.mus.making, play.instr, orch, choir, sonst, spr.deu, betr.tg, migback.s, migback.f, migback.m)
=======
names(d)[names(d) == 'v22b'] <- 'betr.tg' # Taschengeld: Betrag (Problem: Nicht sicher, ob hier Variable v22a (Tschengeld: Rythmus) bereits berücksichtigt wurde

# BIG 5
summary(d$std_big5index)
d <- d %>% group_by( pid ) %>% fill( std_big5index, .direction="downup" )
#d %>% select( school, pid, welle, std_big5index ) %>% 
#  arrange(school, pid, welle) %>% View()
summary(d$std_big5index)

# Emotionale Stabilität
summary(d$std_emostab)
d <- d %>% group_by( pid ) %>% fill( std_emostab, .direction="downup" )
summary(d$std_emostab)

d <- d %>% select( school, pid, welle, lsat, sat_friends, sat_class, treated, female, classsit.prob, classsit.excl, note.deu, note.mat, note.mus, hob.mus.making, hob.mus.lis, dauer.mus.lis, dauer.mus.making, play.instr, orch, choir, sonst, spr.deu,immigr, betr.tg, std_big5index, std_emostab )
>>>>>>> 62a58808dede71cb124d49adebcd42abe05495de:prgs/00_Brainstorm.Rmd
...
<<<<<<< HEAD:paper/Analysis.Rmd
d$mus.active = ifelse(d$play.instr==1 | d$orch==1 | d$choir==1, 1, 0) # Variable is not created because there are factor variables included (Why? When I import the data, the variables are numeric)
d[d==""]<-NA
d <- d %>% group_by(pid) %>% fill(migback.s, migback.f, migback.m)
d$migback <- ifelse(d$migback.s=="1 Deutschland" & d$migback.f=="1 Deutschland" & d$migback.m=="1 Deutschland", 0, 1)
d$migback.s <- NULL
d$migback.f <- NULL
d$migback.m <- NULL
=======
d$mus.active = ifelse(d$play.instr=="Ja" | d$orch=="Ja" | d$choir=="Ja", 1, 0) 
# Variable is not created because there are factor variables included (Why? When I import the data, the variables are numeric)
# 2020-09-07 CW: scheinen doch Faktorvariablen zu sein.
>>>>>>> 62a58808dede71cb124d49adebcd42abe05495de:prgs/00_Brainstorm.Rmd
...
welle1 <- d %>% filter(welle==1) %>% 
  select(school, pid, welle, lsat, sat_friends, sat_class, treated, female, classsit.prob, classsit.excl, note.deu, note.mat, note.mus, hob.mus.making, hob.mus.lis, dauer.mus.lis, dauer.mus.making, play.instr, mus.active, spr.deu, migback)
=======
# fill missing information in wave 2-5
d$immigr[d$immigr==""] <- NA
d$immigr[d$immigr==" - 99 fehlender Wert"] <- NA
d <- d %>% group_by( pid ) %>% fill( immigr, .direction="down" )
d$migback <- factor( 
  if_else(is.na(d$immigr), 3, 
          if_else(d$immigr=="1 Deutschland", 1, 2)))
d$migback <- factor(x=d$migback, labels=c("Native", "Immigrant", "Unkown"))
d <- d %>% arrange(school, treated, pid, welle)
# d %>% select(pid, welle, migback, immigr) %>% View()

# Dataframe for wave 1 (pre-treatment)
welle1 <- d %>% filter(welle==1) %>% 
  select(school, pid, welle, lsat, sat_friends, sat_class, treated, female, classsit.prob, classsit.excl, note.deu, note.mat, note.mus, hob.mus.making, hob.mus.lis, dauer.mus.lis, dauer.mus.making, play.instr, mus.active, spr.deu, immigr, migback)
>>>>>>> 62a58808dede71cb124d49adebcd42abe05495de:prgs/00_Brainstorm.Rmd
...
```{r}
d3 <- d %>% select(school, pid, welle, treated, lsat, sat_friends, sat_class, female, tXw2, tXw3, tXw4, tXw5, migback) %>% arrange( school, treated, pid, welle) %>% ungroup()
table(d3$school, d3$treated)
# saveRDS(d3, file = "../data/d3.RDS")
```

```{r}
d4 <- d %>% select(school, pid, welle, treated, std_big5index, std_emostab, female, tXw2, tXw3, tXw4, tXw5, migback) %>% arrange( school, treated, pid, welle) %>% ungroup()
saveRDS(d4, file = "../data/d4.RDS")
```
...
```{r}
do.call(rbind, dfapply(d3, favstats, 
                       select = c("lsat", "treated", "migback", "female")) )
```
...
sat_friends_pre_treat <- ggplot (welle1, aes(x=school, y=mean.frsat, fill=treated)) + 
  geom_bar (stat="identity", position = "dodge") + coord_cartesian(ylim = c(0, 11)) + labs(title = "Satisfaction with friends", caption = "Mean pretreatment satisfaction (+ 1 sd) with friends (wave 1) for treatment and control group per School") + 
...
```{r}
vars <- c("lsat","female", "sat_friends", "sat_class", "classsit.prob", "classsit.excl", "note.deu", "note.mat", "note.mus", "hob.mus.making", "hob.mus.lis", "dauer.mus.lis", "dauer.mus.making", "play.instr", "mus.active", "migback")
...
prior <- c(prior(normal(0, 5), class = Intercept),
           prior(normal(0, 5), class = b),
           prior(cauchy(0, 1), class = sd))
m3 <- brm( lsat ~ treated + welle + tXw2 + tXw3 + tXw4 + tXw5 + female + migback +
