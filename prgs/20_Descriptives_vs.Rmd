...
lsat_bar <- ggplot(welle1, aes(x=lsat)) +
  geom_bar((aes(y=(..count..)/sum(..count..))), fill = "#011f4b") +
...
su_d <- d %>% group_by(school, welle) %>%
                      summarise( mean.lsat = mean(lsat, na.rm = TRUE))

lsat_vs_time_by_school <- ggplot(su_d, aes(y = mean.lsat, x = welle, group = school)) +
  geom_point(
    aes(color = school), 
    alpha = 0.4,
    size = rel(0.8)
  ...
  labs(x = "Wave", y = "Mean Life Satisfaction",
       caption = "Life Satisfaction through time")

ggsave("../figures/lsat_vs_time_by_school.png", lsat_vs_time_by_school, width = 7, height = 5, units = "in")
  
su_d <- d %>% group_by(school, welle, treated) %>%
                      summarise( mean.lsat = mean(lsat, na.rm = TRUE))

lsat_vs_time_by_school_treated <- ggplot(data = subset(su_d, treated == 1), aes(y = mean.lsat, x = welle, group = school)) +
  geom_point(
    aes(color = school), 
    alpha = 0.4,
    size = rel(0.8)
  ...
  geom_line(aes(color = school)) +
  labs(x = "Wave", y = "Mean Life Satisfaction",
       caption = "Life Satisfaction through time in the treatment group")
...
d %>% filter( welle==5 ) %>% select(school, treated) %>% group_by(treated) %>% table()

kable(d %>% select(school, welle) %>% group_by(school) %>% table())
```
